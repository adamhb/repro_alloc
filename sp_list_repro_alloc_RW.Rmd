...
This code was written by Adam HB to determine the sp list for the reproductive allocation analysis. 
...
Converting Latin names to sp code names: 
...
#looking at distribution of dbh values
sp_n_above50 <- bci.full7 %>%
  filter(DFstatus == "alive" | status == "A") %>%
  count(sp) %>%
  filter( n > 50) %>% select(sp)


sp_n_above50 %>% 
  left_join(bci.full7, by = "sp") %>% 
  filter(DFstatus == "alive" | status == "A") %>%
  select(sp, dbh) %>%
  group_by(sp) %>%
  skim()
...
Assigning species to early vs. late pfts: 
...
Adding Engelbrecht drought indices: 
...
Adding PNAS paper drought designation: 
...
Sp. without drought tolerance data: 
...
Calculating max dbh for each sp using 95th percentile of dbh as "dmax":

```{r}
#calculating Maximum dbh for each species using the 95th percentile of dbh as the "dmax"
...
Function to find basal area from mm and cm:
...
Adding reproductive basal area:
...
rba_df <- data.frame(sp = species_list$sp, r_ba = rep(0,length(species_list$sp)), ba = rep(0,length(species_list$sp)), r_abun = rep(0,length(species_list$sp)))
                     #, n_ind = rep(0, length(species_list$sp)))
...
for(i in 1:nrow(species_list)){
  ...
  # rba_df[i, 5] <- bci.full7 %>%
  #   select(sp, status) %>%
  #   filter(sp == spp, 
  #          status == "A") %>%
  #   group_by(sp) %>% summarise(., n())
...
```{r}
species_list_for_joe

rep_alloc_list_5.18.19 <- species_list_final %>% select(sp, Latin, dmax, e_vs_l, dpft, pft, r_ba, ba, r_abun)
#this has all cols including r_abun, r_ba, and ba
#would be nice to add col for # indv.
rep_alloc_list_5.18.19

write.csv(rep_alloc_list_5.18.19, file = "rep_alloc_list_5.18.19.csv")
